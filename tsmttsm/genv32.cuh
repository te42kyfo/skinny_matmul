...
bool tsmttsm(const int blockCount, const int varM, const int varN, const int K,
  ...
  /*  if (N > M) {
    GENV32::blockProductKernel<T, N, M, 256, true><<<blockCount, 256>>>(
        B, A, (T *)d_temp_storage, K, ldb, lda, ldc);
        } else {*/
  GENV32::blockProductKernel<T, M, N, 256, false><<<blockCount, 256>>>(
      A, B, (T *)d_temp_storage, K, lda, ldb, ldc);
  //}
  GENV32::deviceReduce<T, M, N><<<M * N / 256 + 1, 256>>>(
